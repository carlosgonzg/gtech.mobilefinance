...
.controller('SessionCtrl', function ($scope, $rootScope, $location, $window, User) {
    ...
    $rootScope.$on("$routeChangeStart", function () {
        if (!$window.sessionStorage.isAuthenticated && ($location.path() != '/login' && $location.path() != '/register' && $location.path().substr(0, 15) != "/changepassword" && $location.path().substr(0, 14) != "/confirm/email")) {
            $location.path('/login');
        }
    });
});
